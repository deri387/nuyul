...
let scrape = async () => {
	...
	async function openWeb(btc) {
		console.log('\x1b[31m', 'Hampir Selesai...');
		console.log('\x1b[31m', 'Masukkan Alamat BTC anda : 1JiLigaE13bYwP4nbGZfbhprX2DE6riT81');
	...
	async function waitTime(){
		for (var i=0;i<8000000;i++){
			...
			if(cekBro=="Claim Now"){
				...
				console.log('\x1b[31m','Total Pendapatan');
				await page.evaluate(()=> {
	...
	async function habis(){
		...
		console.log('\x1b[31m',"Sisa claim : 0, " + bal);
		console.log('\x1b[31m',"Script akan berhenti dan mengecek otomatis 1 jam ke Depan");
		await page.waitFor(3600000);
	}
	
	if(btcaddress==""){
		console.log('\x1b[31m',"Silahkan Masukkan BTC anda!");
		return "btc_kosong";
	}
		...
		for(var cekIn=0;cekIn<1000000;cekIn++){
			...
			if(sisaClaim=="err_loop"){
				...
				for(var ii=0; ii<sisaClaim;ii++){
					...
					console.log('\x1b[31m',"Sisa Claim : " + await cekLoop() + ", " + bal);
					if(await waitTime()=="Captcha"){
						...
						console.log('\x1b[31m','Enter untuk buka captcha dan verifikasi manual');
						await prompt.start();
						...
						if(isCaptchaGone.SiapLanjut==""){
							...
							console.log('\x1b[31m','Tutup browser dan tekan Enter untuk lanjut setelah captcha hilang');
							await prompt.start();
 ...
 scrape().then((value) => {
	if(value=="btc_kosong"){
		console.log('\x1b[31m',"Tutup dan jalankan kembali script ini setelah anda memasukkan BTC anda di file tersebut.");
	}else if(value=="kurang"){
		console.log('\x1b[31m','Coin kurang.');
	}
