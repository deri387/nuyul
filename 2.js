...
let scrape = async () => {
	...
	console.log("%cAuto Claim BTC %s", css, 'Thanks To : Aneizal , Edited By Deri Komara');
	...
	async function openWeb(btc) {
		console.log('%c Hampir selesai... ', 'background: #222; color: red');
		console.log('%c Masukkan Alamat BTC anda : 1JiLigaE13bYwP4nbGZfbhprX2DE6riT81','background: #222; color: red');
	...
	async function waitTime(){
		for (var i=0;i<8000000;i++){
			...
			if(cekBro=="Claim Now"){
				...
				console.log('%c Total Pendapatan','background: #222; color: red');
				await page.evaluate(()=> {
	...
	async function habis(){
		...
		console.log("%c Sisa claim : 0, ",'background: #222; color: red' + bal);
		console.log("%c Script akan berhenti dan mengecek otomatis 1 jam ke Depan",'background: #222; color: red');
		await page.waitFor(3600000);
	}
	
	if(btcaddress==""){
		console.log("%c Silahkan Masukkan BTC anda!",'background: #222; color: red');
		return "btc_kosong";
	}
		...
		for(var cekIn=0;cekIn<1000000;cekIn++){
			...
			if(sisaClaim=="err_loop"){
				...
				for(var ii=0; ii<sisaClaim;ii++){
					...
					console.log("%c Sisa Claim : ",'background: #222; color: red' + await cekLoop() + ", " + bal);
					if(await waitTime()=="Captcha"){
						...
						console.log('%c Enter untuk buka captcha dan verifikasi manual','background: #222; color: green');
						await prompt.start();
						...
						if(isCaptchaGone.SiapLanjut==""){
							async function openCaptcha(btc) {
								console.log('%c Mohon Tunggu...','background: #222; color: red');
							...
							console.log('%c Tutup browser dan tekan Enter untuk lanjut setelah captcha hilang','background: #222; color: red');
							await prompt.start();
